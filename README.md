...
- 设置页可实时切换始终置顶、位置锁定和当前用户开机启动；锁定后仍可点击晓曦收展。
- 窗口位置、始终置顶和位置锁定自动保存到当前用户的本地配置，并在重启后恢复。
- Windows 通知区提供晓曦托盘图标；右键可显示/隐藏、展开/收起、切换置顶和位置锁定、打开设置或退出。
- 主窗口不在任务栏显示；程序启动和窗口隐藏后都通过通知区托盘图标恢复。
- 左击托盘图标或再次启动程序会恢复已有窗口；第二次启动不会创建另一套监控线程。
- 窗口会选择最近的 Windows 工作区并至少保留 32 个物理像素可见，用于处理任务栏、多屏移除和 DPI 变化。
- 收起态允许贴近屏幕边缘；展开时会自动移入当前工作区，收起后恢复展开前的收起态位置。
- 监控、对话、设置三个基础页签。
- CPU、物理内存、GPU、显存、ACPI 热区温度和非回环网卡上下行速率每 2 秒真实刷新。
- GPU 利用率取整机最忙物理引擎；显存显示当前最活跃硬件适配器的总 GPU 内存占用。
- 温度显示当前最高有效 ACPI 热区；它不等同于 CPU 核心温度，不支持的设备会明确显示不可用。
- 已完成运行时无关的智能体连接器契约、无网络 HTTP 流模拟器和真实子进程 CLI 原型。

本地配置使用同目录临时文件和 Windows 原子替换；损坏配置会保留 `.corrupt` 副本并回退默认值。开机启动使用当前用户 `Run` 注册表项，不需要管理员权限，也不重复写入 JSON 配置。
